...
import ru.noxen.common.util.item.ItemUtil;
...
public class TargetHud extends AbstractDraggable {
    ...
    @Override
    public void tick() {
        LivingEntity hoveredTarget = mc.crosshairTarget instanceof EntityHitResult result
                && result.getEntity() instanceof LivingEntity livingEntity
                && !livingEntity.isInvisible() ? livingEntity : null;

        if (hoveredTarget != null) {
            lastTarget = hoveredTarget;
            startAnimation();
            stopWatch.reset();
